...
package com.flyou.pictureselecter.widget.subscaleview;
...
import com.flyou.pictureselecter.widget.subscaleview.decoder.CompatDecoderFactory;
import com.flyou.pictureselecter.widget.subscaleview.decoder.DecoderFactory;
import com.flyou.pictureselecter.widget.subscaleview.decoder.ImageDecoder;
import com.flyou.pictureselecter.widget.subscaleview.decoder.ImageRegionDecoder;
import com.flyou.pictureselecter.widget.subscaleview.decoder.SkiaImageDecoder;
import com.flyou.pictureselecter.widget.subscaleview.decoder.SkiaImageRegionDecoder;
...
public class SubsamplingScaleImageView extends View {
    ...
    private void doubleTapZoom(PointF sCenter, PointF vFocus) {
        ...
        // modify by wanjing on 2016/12/7 10:28 双击缩放的范围应该是(xscale, maxscale).
//        float targetScale = zoomIn ? doubleTapZoomScale : minScale();
        float targetScale = zoomIn ? doubleTapZoomScale : (getWidth() - (getPaddingLeft() + getPaddingRight())) / (float) sWidth();
        // end modify on 2016/12/7 10:28.
    ...
    private void fitToBounds(boolean center, ScaleAndTranslate sat) {
        ...
        // add by wanjing on 2016/10/31 15:24 初始加载图片的时候，计算最大缩放比, 并使图片宽度适配屏幕宽度显示。
        if ((sat.scale >= -0.00001) && (sat.scale <= 0.00001)) { //sat.scale == 0; 用来判断是否是初始加载图片。
            float xScale = (getWidth() - (getPaddingLeft() + getPaddingRight())) / (float) sWidth();
            float yScale = (getHeight() - (getPaddingTop() + getPaddingBottom())) / (float) sHeight();
            if (xScale <= 1 || yScale <= 1) { // 计算图片最大缩放比，没什么依据，自己设定的值。
                this.doubleTapZoomScale = this.maxScale = xScale * 4;
            } else if (xScale <= 2 || yScale <= 2) {
                this.doubleTapZoomScale = this.maxScale = xScale * 3;
            } else if (xScale <= 4 || yScale <= 4) {
                this.doubleTapZoomScale = this.maxScale = xScale * 2;
            } else {
                this.doubleTapZoomScale = this.maxScale = xScale;
            }

            sat.scale = xScale;
        }
        // end add on 2016/10/31 15:24.
    ...
    private void fitToBounds(boolean center) {
        ...
        // modify by wanjing on 2016/11/15 10:02 为了保证图片从左上角开始缩放, 并且图片不偏移。
//        if (init) {
//            vTranslate.set(vTranslateForSCenter(sWidth()/2, sHeight()/2, scale));
//        }
        if (init || (sHeight() * scale < getResources().getDisplayMetrics().heightPixels
                    && sWidth() * scale < getResources().getDisplayMetrics().widthPixels)) {
            vTranslate.set(vTranslateForSCenter(0, 0, scale));
        }
        // end modify on 2016/11/15 10:02.
    }
    ...
    private float minScale() {
        ...
        if (minimumScaleType == SCALE_TYPE_CENTER_CROP) {
            ...
            // modify by wanjing on 2016/11/29 9:45 解决小图不能缩放的问题。
//            return Math.min((getWidth() - hPadding) / (float) sWidth(), (getHeight() - vPadding) / (float) sHeight());
            float scale = Math.min((getWidth() - hPadding) / (float) sWidth(), (getHeight() - vPadding) / (float) sHeight());
            if (sHeight == 0 && sWidth == 0){
                return scale; // 无限大，不晓得什么用，先保留。
            } else {
                return Math.min(1.0f, scale);
            }
            // end modify on 2016/11/29 9:45.
        }
    }
